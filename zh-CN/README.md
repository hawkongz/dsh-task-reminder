...
**出错停止**（api-session/error，如 400 的红色错误）。同一次停止只报一次（错误
优先：完成弹窗会等一个合并窗口，同一次停止的错误到达就让位）。弹窗时机二选一：
...
* **Windows 系统弹窗，唯一视觉通道：** 走 Web Notification API，以操作系统 toast
  的形式弹出，应用退到后台也看得到；点击弹窗窗口回前台并打开对应会话。三种停止
  原因都覆盖：**任务完成**、**等你回答**（Agent 阻塞在 ask_user_question / 计划
  评审——只读 `uiSession.sessionStatus` 快照的出现边沿，绝不参与 user-questions
  应答链）、**出错停止**（`api-session/error`，如 400 的红色错误）。同一次停止
  只报一次、错误优先：完成弹窗先等一个合并窗口，同一次停止的错误到达即让位。
...
去重、两种弹窗时机、三种停止原因（完成 / 等你回答 / 出错）与「一次停止只报一次、
错误优先」的合并、提示音每次停止都响（与窗口状态无关）、五个配置的默认值 / 读 /
写 / 恢复默认、每种音效与音量对应的振荡器参数、系统通知的权限路径与设置页里的
「申请通知权限」按钮、挂起 AudioContext 的手势拉活，以及资源回收。
