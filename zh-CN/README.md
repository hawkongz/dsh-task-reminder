...
* **系统通知默认开启：** 走 Web Notification API，以操作系统 toast 的形式弹出，
  应用退到后台也看得到；点击通知窗口回前台并打开对应会话。首次装载时代码会替
  用户申请一次权限（浏览器的 Notification API 没有免权限通道——只在页面内画东西
  的插件才不需要这一步）；之后权限若被重置，设置页「系统通知」行下的「申请通知
  权限」按钮可以再申请。被拒绝或不支持时，设置页给出明确提示，而不是假装生效。
...
> 通知权限申请，选「允许」即可。随便找个会话跑一个任务，切到别的应用，等着收提醒。
...
* **系统通知不弹。** 看 `__dshTaskReminder.state()`：`notificationSupported`
  必须为 `true`、`notificationPermission` 必须为 `granted`。如果权限还是
  `default`（一次都没问过），设置页「系统通知」行下有「申请通知权限」按钮，
  点一次再选「允许」；如果是 `denied`（之前点了阻止），去浏览器地址栏的站点
  权限里允许通知，再点一次那个按钮。
